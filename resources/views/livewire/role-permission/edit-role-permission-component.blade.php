  ...
  <div>
    <div class="content container-fluid">
      ...
      <div class="alert alert-{{ $msg == 'error' ? 'danger' : $msg }} alert-dismissible fade show" role="alert">
        <i
          class="fas fa-{{ $msg == 'success' ? 'check' : ($msg == 'error' ? 'exclamation' : 'info') }}-circle me-2"></i>
        {{ session($msg) }}
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h4 class="card-title mb-0">Edit Role: {{ $roleName }}</h4>
            </div>

            <div class="card-body">
              <form wire:submit.prevent="updateRolePermission">
                <!-- Role Name Input -->
                <div class="mb-3">
                ...
                <div class="mb-2">
                  ...
                  <div class="d-flex justify-content-end mb-1">
                    ...
                    <button type="button"
                      class="btn btn-sm {{ $this->areAllPermissionsSelected() ? 'btn-danger' : 'btn-success' }}"
                  ...
                  <div class="table-responsive border rounded" style="max-height: 600px; overflow-y: auto;">
                    <table class="table table-bordered table-sm align-middle mb-0">
                      <thead class="table-light sticky-top custom-table-header">
                        <tr>
                          <th style="width: 40%;">Element/Page Name</th>
                          <th class="text-center" style="width: 15%;">Sidebar Access</th>
                          <th class="text-center" style="width: 45%;">Page Operations</th>
                        </tr>
                      </thead>
                      <tbody>
                        @php
                        ...
                        foreach ($item['operations_array'] ?? [] as $op) {
                        $ops[] = $op;
                        }
                        }
                        }
                        ...
                        $hasSidebarAccess = $this->isSidebarAvailable($elementName);
                        @endphp
                        <tr class="parent-row">
                          <td><strong class="text-primary"><i class="fas fa-folder me-2"></i>{{ $elementName }}</strong>
                          </td>
                          <td class="text-center">
                            @if($hasSidebarAccess)
                            <input type="checkbox" class="form-check-input custom-checkbox"
                              wire:model.live="sidebarPermissions" value="{{ $elementName }}"
                              id="sidebar_{{ $parent['id'] }}" @if($this->isSidebarSelected($elementName)) checked
                            @endif>
                            @else
                          ...
                          <td class="text-center">
                            @if($this->shouldShowPageOperations($elementName))
                            <span class="text-muted small">—</span>
                            @else
                            <span class="text-muted small">—</span>
                            @endif
                        ...
                        $hasSidebarAccess = $this->isSidebarAvailable($childElement);
                        @endphp
                        <tr class="child-row">
                          <td class="ps-4">
                            <span class="text-secondary">
                              ↳ <i class="fas fa-file me-2"></i>{{ $childElement }}
                            </span>
                          </td>
                          <td class="text-center">
                            @if($hasSidebarAccess)
                            <input type="checkbox" class="form-check-input custom-checkbox"
                              wire:model.live="sidebarPermissions" value="{{ $childElement }}"
                              id="sidebar_{{ $child['id'] }}" @if($this->isSidebarSelected($childElement)) checked
                            @endif>
                            @else
                            <span class="text-muted small">—</span>
                            @endif
                            ...
                            <div class="d-flex flex-wrap gap-2">
                              ...
                              <div class="form-check">
                                ...
                                <label class="form-check-label small"
                                  for="perm_{{ Str::slug($childElement) }}_{{ $op }}">
                                  {{ ucfirst($op) }}
                                </label>
                              </div>
                              @endforeach
                        ...
                        <tr class="standalone-page">
                          <td>
                            <strong class="text-info">{{ $pageName }}</strong>
                          </td>
                          <td class="text-center">
                            @if($hasSidebarAccess)
                            <input type="checkbox" class="form-check-input custom-checkbox"
                              wire:model.live="sidebarPermissions" value="{{ $pageName }}"
                              id="sidebar_standalone_{{ Str::slug($pageName) }}"
                              @if($this->isSidebarSelected($pageName))
                            checked @endif>
                            @else
                          ...
                          <td>
                            ...
                            <div class="d-flex flex-wrap gap-2">
                              ...
                              <div class="form-check">
                                ...
                                checked @endif>
                                <label class="form-check-label small" for="perm_{{ Str::slug($pageName) }}_{{ $op }}">
                                  {{ ucfirst($op) }}
                                </label>
                              </div>
                              @endforeach
                  ...
                  <div class="text-danger mt-2 small"><i class="fas fa-exclamation-triangle me-1"></i>{{ $message }}
                  </div>
                  @enderror
                ...
                <div class="text-end mt-4">
                  <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
                    <span><i class="fas fa-save me-1"></i> Update Role</span>
                    {{-- <span ><i class="fas fa-spinner fa-spin me-1"></i> Updating...</span> --}}
                  </button>
