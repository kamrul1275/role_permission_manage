...
<div class="content container-fluid">
  ...
  <div class="page-header">
    <div class="row">
      <div class="col">
        <h3 class="page-title">Assign Permissions to User</h3>
        <ul class="breadcrumb">
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title mb-0">Assign User Permissions</h4>
        </div>

        <div class="card-body">
          <form wire:submit.prevent="storeUserPermission">

            <!-- User Selection -->
            <div class="mb-4">
              <label for="user_id" class="form-label fw-semibold">Select User</label>
              <select wire:model.live="user_id" id="user_id" class="form-select @error('user_id') is-invalid @enderror" required>
            ...
            <div class="mb-4">
              ...
              {{-- <div class="d-flex justify-content-end mb-3">
                ...
                <button type="button" class="btn btn-sm {{ $this->allPermissionsSelected ? 'btn-danger' : 'btn-success' }}" wire:click="selectAllPermissions">
                  <i class="fas fa-{{ $this->allPermissionsSelected ? 'minus' : 'check' }} me-1"></i>
                  {{ $this->allPermissionsSelected ? 'Deselect All' : 'Select All' }}
                </button>
              </div> --}}
                            ...
                            <tr>
                              ...
                              <td class="text-center">
                                <input type="checkbox"
                                       ...
                                       @if($this->isSidebarSelected($elementName)) checked @endif
                                    ...
                                    @foreach ($pageWisePermissionData[$elementName] as $permGroup)
                                      @foreach ($permGroup['operations_array'] ?? [] as $op)
                                        ...
                                        <div class="form-check">
                                          <input type="checkbox"
                                                 class="form-check-input custom-checkbox-small page-checkbox"
                                          ...
                                          <label class="form-check-label small"
                                                 for="perm_{{ Str::slug($elementName) }}_{{ $op }}">
                                            {{ ucfirst($op) }}
                                          </label>
                                        </div>
                                      @endforeach
            ...
            <div class="text-end mt-4">
              <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
                <span wire:loading.remove><i class="fas fa-save me-1"></i> Save User Permissions</span>
...
{{-- 
  <style>
    ...
    .custom-checkbox-small {
      transform: scale(1.1);
      border: 1px solid #100c0c !important;
      cursor: pointer;
    }

    .custom-checkbox:checked,
    .custom-checkbox-small:checked {
      background-color: #FE8C1F !important;
      border-color: #FE8C1F !important;
    }
  </style>
 --}}
